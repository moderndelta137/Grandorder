...
「了解。初夜戦へ入ります」`,
  ...
  chapter2_main_001: {
    ...
    text: (s) => `10月2日 13:10。学園本棟の廊下は昼の雑談で満ちている。
その中で、昨夜の火薬臭だけを覚えている目線がこちらを追ってきた。

「${s.servant.className}、監督役から呼び出しだ。第三交差点で会談を開くと言っている」
「休戦の提案に見せた、戦力確認ですね」
「断れば臆病と取られ、受ければ腹を読まれる。どちらにせよ試される」`,
    choices: [
      {
  ...
  chapter2_main_002: {
    ...
    text: (s) => `10月2日 13:40。監督役室は香だけが濃く、声は意図的に抑えられていた。

「会談は18時40分、第三交差点。議題は期限付き休戦」
「期限を聞こう」
「明朝まで。違反時は即時で交戦再開」

「${s.servant.className}、この条件は」
「時間を区切って布陣を探る形です。拾える情報は多いですが、信用の担保はありません」`,
    choices: [
      {
        label: "会談に応じる（情報優先）",
        effect: (s) => {
  ...
  chapter2_main_003: {
    phase: "章本文",
    title: "第2章 本文: 夕刻への移動",
    text: (s) => `10月2日 16:50。学園裏門を抜け、市街地の南へ降りる。
人通りは薄く、代わりに窓越しの視線と足音の間隔が揃い始める。

「${s.servant.className}、尾行の数は」
「二組。干渉せず、こちらの速度だけを測っています」
「会談前に腰が引けるか試してるわけだ」
「はい。ここで歩幅を崩すと、席に着く前に主導を失います」`,
    choices: [
      {
  ...
  chapter2_main_004: {
    ...
    text: `10月2日 18:40。第三交差点の街灯が、互いの表情から温度だけを奪っていく。

「今夜は刃を引く」
「期限は明朝まで。違反すれば即時で破棄」
「長い休戦は裏切りの準備になる」
「だから短く切った。互いに猶予を持たせないためだ」

握手は交わした。だが、指先の力は最後まで抜けなかった。
合意できたのは信頼ではなく、被害をどこまで許すかという線引きだけだ。`,
    choices: [
      {
  ...
  chapter2_main_005: {
    phase: "章本文",
    title: "第2章 本文: 帰路の再評価",
    text: (s) => `10月2日 20:15。学園裏門へ戻る坂道で、被害見積もりをもう一度引き直す。
守る範囲を広げれば手数が足りない。先制を急げば露見と摩耗が増える。

「${s.servant.className}、ここで運用を固定する」
「はい、マスター。休戦は短い。今夜の配分が明日の戦場配置を決めます」`,
    choices: [
      {
        label: "同盟を維持し被害を抑える",
        effect: (s) => {
  ...
  chapter2_main_006: {
    phase: "章本文",
    title: "第2章 本文: 夜への繋ぎ",
    text: (s) => `10月2日 21:05。工房へ戻ると、新しい監視ログが机を埋めていた。
休戦は生きている。だが静かな夜ほど、破られる瞬間は近い。

「${s.servant.className}、夜警の配置を詰める。明日は言葉より先に動きで示す」
「了解です、マスター。今夜の準備をそのまま明朝の初動へ接続します」`,
    choices: [
      {
        label: "第2章を終え、次行動へ",
        effect: (s) => {
