...
### Sprint 5（V1仕上げ）
- 成果物: シナリオ完成版（総文字数5万字管理 + 読後感調整）
- 実装計画: `docs/SPRINT5_PLAN.md`
- 目的: V1受け入れ基準「シナリオ主導で1周完結可能」を本文密度で担保
- 保守: ボタン押下感 / シーン本文遷移 / サーヴァント詳細ウィンドウ開閉のUI演出を改善（ロジック非変更）。
...
- Sprint 4: 完了（Step A〜D実装/検証済み）
- Sprint 5: 進行中（V1シナリオ仕上げ + UI演出微調整）


## 4.6 Sprint 4実装スコープ（昼ランダムイベント完全版）

詳細は `docs/SPRINT4_PLAN.md` を正とし、本章はV1全体計画上の拘束条件を定義する。

### Must
1. 4カテゴリ抽選（共通 / マスタービルド専用 / 味方真名専用 / 敵生存真名専用）
2. カテゴリ重み固定（共通 > 味方真名 > ビルド > 敵生存真名）
3. 選択肢あり/なしイベントの双方実装
4. 低リスク効果（理想点・看破・同盟・一般被害・魔力）の安定適用
5. 既存の夜戦・救済・4エンディングの非破壊回帰

### Should
6. 味方真名イベントの拡張（1騎あたり2本目）
7. 中リスク効果（短期バフ/デバフ、関係値変動）の限定導入

### Won’t（Sprint 4では実装しない）
8. 契約変更の恒久化ロジック
9. 宝具覚醒の本実装
