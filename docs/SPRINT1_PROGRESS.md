...
### 2.39 第1章接続仕様の明確化（登場人物 + イベント挿入有無）
- `docs/PLOT_MASTER.md` に 3.1F を追加し、第1章各区間の登場人物と「通常イベントを挟むか」を表形式で明示。
- 第1章は `chapter1_main_006` まで固定本文を連続運用し、通常処理は `dayAction` 接続後に実行する方針を明文化。

### 2.40 第2章接続仕様の明確化（登場人物 + イベント挿入有無）
