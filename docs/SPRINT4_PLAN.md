...
- [x] Step A-1: `day_events.csv` と生成処理追加
- [x] Step A-2: `scenario.js` に抽選と適用関数追加（選択肢なしイベントの土台）
- [x] Step A-3: no-op フォールバック実装
- [x] Step B-1: 共通イベント12本以上を投入（no-op除く）
- [x] Step B-2: ビルド専用6本（完了）
- [x] Step B-3: サーヴァント専用（味方7 + 敵6）
- [x] Step C-1: 2〜3択の期待値バランス調整（主要イベント反映）
- [x] Step C-2: 同盟/被害/看破ログの読解性向上（変動サマリ導入）
- [x] Step D: 回帰確認（D-1〜D-3完了）
...
### Step D: 回帰確認
- [x] D-1: 1000回シミュレーションでカテゴリ出現比率を検証
- [x] D-2: セーブ/ロード互換確認
- [x] D-3: 既存4エンディング到達性の回帰確認
...
- `node scripts/simulate_day_events.mjs`（新規）
  ...
  - 抽選失敗率0%を確認
- `node scripts/validate_sprint4_d2.mjs`（新規）
  - 現行/旧形状セーブの読込互換を検証
  - 互換外versionの拒否を確認
- `node scripts/validate_sprint4_d3.mjs`（新規）
  - 既存4エンディング判定の回帰を検証
  - Sprint3 D-3 基準チェックの非破壊を確認
...
1. `data/csv/day_events.csv` 新規作成（カテゴリ/条件/結果定義）
2. `scripts/generate_csv_data.mjs` に day_events 取り込み処理追加
3. `src/data/generatedData.js` 生成スキーマへ `DAY_EVENTS` 追加
4. `src/scenario.js` にカテゴリ抽選・イベント抽選・適用関数追加
5. `src/game.js` で日中イベント本文/選択肢表示をサポート
6. `scripts/simulate_day_events.mjs` 追加（1000周分布 + 失敗率監視）
7. 既存回帰（戦闘/召喚/D-2/D-3）を再実行

## 11. リスクと回避策（Sprint 4）

- リスク1: ランダムイベント導入で日次テンポが低下
  - 対策: 1イベントの本文は 120〜320字目安、選択肢は最大3択。
- リスク2: 状態更新の競合で既存分岐が不安定化
  - 対策: 効果適用は `applyDayEventOutcome()` に集約し、副作用を一元管理。
- リスク3: 重み設計が偏り、カテゴリ体験が固定化
  - 対策: シミュレーション結果でカテゴリ出現比率の許容レンジ（±7%）を監視。
- リスク4: 真名専用イベントの設定不整合
  - 対策: 真名IDをデータキー化し、クラス名依存分岐を禁止。

## 12. Sprint 4 完了成果物（Definition of Done）

1. 実装物: `day_events.csv`, `simulate_day_events.mjs`, シナリオ統合コード
2. ドキュメント: `docs/SPRINT4_PLAN.md` 実績欄更新、`docs/SPRINT1_PROGRESS.md` 追記
3. 検証: 自動検証 + 手動3観点（選択肢あり/なし、章差分、敵生存条件差分）
4. 回帰: `simulate_battle/simulate_summon/validate_sprint3_d2/validate_sprint3_d3` 全通過
